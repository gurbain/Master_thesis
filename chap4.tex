...
In this chapter, the results obtained in the laboratory and during an \gls{RGA} test sessions are presented and analyzed in order to make conclusions about the performance of the algorithms elaborated in this project. The two first sections put forth the sensors performance and give a qualitative appreciation for each of them motivating the use of a fusion algorithm. The third section takes an interest in the full system calibration and, through images and measurements, highlights the accuracy of the algorithm. Finally, in the fourth section, we show the current results from the multi-sensor fusion algorithm obtained during ground test sessions and also briefly \gls{RGA} sessions. We then try to explain why they may be not as good as expected theoretically.

\section{ORF Acquisition}

The \textit{\gls{ORF} C++ \gls{API}} as proposed by the constructor MESA-Imaging allows the user to get three different images in a single capture:
\begin{itemize*}
\item A \textit{depth image} $D_T$ coded in 10 bits from which a distance in meters can be computed for each pixel.
\item A \textit{visual image} of the environment $V_T$ coded in 16 bits.
\item A \textit{confidence} image acting as a indicator of confidence in the depth measurement coded in 16 bits.
\end{itemize*}
...
Figure \ref{fig:manip_0_0_1} shows three typical images captured in a stationary environment. We can see that further the distance, the lighter the depth picture. We can also notice the increasing noise farther from the center of the camera, the lower confidence on the objects edges (due to discontinuities) and the poor quality of the visual image (partly due to image equalization to adapt to changing lighting conditions).
...
\end{subfigure}
\caption{\textit{Left}: depth image - the darker, the nearer. \textit{Center}: visual image. \textit{Right}: confidence image - the variance is high on objects rims (discontinuities), on the picture edges (sensor limitations) and on some surfaces (thermal noise).}
...
\end{subfigure}
\caption{\textit{Left}: depth. \textit{Center}: visual. \textit{Right}: confidence. When the object exceed the range limitation of the camera, the measurement can be twisted.}
\label{fig:manip_0_0_2}
\end{center}
\end{figure}

\noindent
In figure \ref{fig:manip_0_0_3}, the same object is held a little farther so it can be measured correctly. However, the low luminosity induces the automatic exposure regulation to wait a little longer during the capture in order to make a good measurement, leading to very noisy pictures where the environment is brighter.
% figure manip_0_0_3
\begin{figure}[!htt]
\begin{center}
\begin{subfigure}{5cm}
...
\noindent
Figure \ref{fig:manip_0_0_4} illustrates that the quality of the measurement is material-dependent. Here, the reflection of an aluminum slab corrupts the depth and visual pictures when the computation of confidence does not highlight the problem everywhere on the slab which means that the measurement of the quality cannot rely on the confidence picture only.
...
\noindent
Let us also mention the importance of the \gls{ORF} IR emission time. In figure \ref{fig:manip_0_0_4_bis}, we compare \textit{confidence images} taken in the same conditions but where the IR emits either very shortly during the capture, either constantly. 

% figure manip_0_0_4_bis
\begin{figure}[!htt]
\begin{center}
\begin{subfigure}{5cm}
  \centering
  \includegraphics[width=4.8cm]{img/manip_0_0_4_d.png}
\end{subfigure}%
\begin{subfigure}{5cm}
  \centering
  \includegraphics[width=4.8cm]{img/manip_0_0_4_e.png}
\end{subfigure}
\caption{\textit{Left}: short IR emissions during capture leads to bad confidence image. \textit{Right}: leaving the IR emitter constantly ON produce a better confidence.}
\label{fig:manip_0_0_4_bis}
\end{center}
\end{figure}

\noindent
Last but not least, the problem of relative motion between the camera and the environment is presented in figure \ref{fig:manip_0_0_5}. On those pictures, a checkerboard is moved with a speed barely higher than a few centimeters per second and leads to very bad results. This can be a very important issue in space where the studied system is supposed to track and analyze the properties of spinning and tumbling targets.
...
In figure \ref{fig:manip_0_1_1}, many typical pictures are taken in the limits of the space where checkerboards can be detected using only the \textit{visual image}. 
...
\end{subfigure}
\caption{We vary the checkerboard orientation to the limits of detection to cover the space at maximum. \textit{Left}: the farthest distance. \textit{Center}: the maximal inclination. \textit{Right}: the closest distance.}
\label{fig:manip_0_1_1}
\end{center}
\end{figure}

\noindent
From the coordinates of the points, the software gives the following \textit{intrinsic matrix}:
\begin{equation}
\begin{pmatrix}[0.8]
532.74 & 0 & 308.64\\
0 & 490.43 & 222.22\\
...
\end{subfigure}
\caption{\textit{Left}: a checkerboard before rectification. \textit{Right}: the same checkerboard after the distortions rectification. The edges are now straight and parallel.}
...
\end{equation}
Which is once again the same rough accuracy of a few millimeters compared to the $7.12cm$ we were supposed to obtain.
%Figure manip_0_3_2
\begin{figure}[!htt]
\begin{center}
\begin{subfigure}{5cm}
...
In this last experiment with the \gls{ORF}, a point cloud is constructed using the \textit{\gls{PCL} library} tools included in the software developed in this project. In figure \ref{fig:manip_0_4_1}, two views of the same points cloud are given. We can notice that the \textit{visual image} is included in the process since we can see the pattern on the checkerboard. The importance of the scattering and the thermal noise can be highlighted especially if we look precisely to the edges of the checkerboard and its shape from above (which is supposed to be flat).\\
...
\noindent
Figure \ref{fig:manip_0_4_2} illustrates the noise significance due to auto-exposure when an object is very close to the lens.
...
The reconstruction of a 3D points cloud from stereo sensors is performed through the elaboration of a \textit{disparity map}, whose information is quite similar to that of the \gls{ORF} depth map. To create this \textit{disparity map}, different techniques can be used, a taxonomy of those is given in \cite{stereo_methods_comparaison}. Basically, the process always implies the following steps: \textit{features detection and matching}, \textit{aggregation}, \textit{disparity computation} and \textit{optimization}. The quality of the \textit{disparity image} is dependent on the number of matchable features, called \textit{supporting points}, in the left and right pictures: if the considered environment is highly textured, this will lead to accurate 3D reconstruction while uniform patches will not give reliable results. The interstices between \textit{supporting points} is filled by \textit{aggregation} methods around those points which have also a lot of influence on the final \textit{disparity map}. In figure \ref{fig:manip_1_1}, the results of a block matching stereo function provided in \textit{OpenCV} libraries illustrate our remarks.
...
\end{subfigure}
\caption{\textit{Left}: left image. \textit{Center}: right image. \textit{Right}: disparity map reconstructed with \textit{OpenCV} stereo block matching function - the checkerboard (high textured) gives a good result when the patch in the bottom right corner (uniform) is badly represented.}
\label{fig:manip_1_1}
\end{center}
\end{figure}
...
\end{subfigure}
\caption{\textit{Above}: visual, confidence and depth images captured with the ORF - the movement induces a error in the depth measurement. \textit{Below}: left and right images of the stereo sensor and the disparity map - even with the movement, the result is acceptable for the checkerboard.}
\label{fig:manip_1_2}
\end{center}
\end{figure}

\noindent
In figure \ref{fig:manip_1_2}, the movement reliability is highlighted: a checkerboard animated with a speed of the few dozen of centimeters per second is captured by the two sensors. Even if the stereo images could be considered a little more blurred than in the static case, they are still sufficient to detect features and a \textit{disparity map} can be more reliable for textured pattern than the \gls{ORF} \textit{depth map}.
% Figure manip_1_3
\begin{figure}[!htt]
\begin{center}
\begin{subfigure}{4.2cm}
...
\end{subfigure}
\caption{\textit{Above}: visual, confidence and depth images captured with the ORF - as the checkerboard is too close, the depth measurement is very bad. \textit{Below}: left and right images of the stereo sensor and the disparity map - the checkerboard depth measurement is still correct near the camera.}
\label{fig:manip_1_3}
\end{center}
\end{figure}
...
\end{subfigure}
\caption{\textit{Above}: a bad capture for calibration (from left to right: left image, right image, depth image, confidence image, ORF visual image). \textit{Below}: a good capture (same disposition).}
\label{fig:manip_2_2}
\end{center}
\end{figure}
...
\end{center}
\end{figure}
It is important to understand though that this has no physical meaning, it is just a way to verify a few parameters like the size of the cloud, the regularity of the checkerboard, their straightness or the noise. For example, we can notice the noise is more important with the \gls{ORF} as predicted theoretically (figure \ref{fig:manip_2_4}). The RANSAC scheme will help to put aside the points that are too distant each other. The points evaluated by the stereo sensors are more accurate, since the triangulation process is based on \textit{feature matching}, which is an easier problem for checkerboard corner detection. Hence, the accuracy is driven by the theoretical definition of equation (\ref{eq:stereo_acc}), though the \textit{focal length} and the \textit{baseline} values depend on the stereo calibration efficiency.
% Figure manip_2_4
\begin{figure}[!htt]
\begin{center}
\begin{subfigure}{7cm}
...
\end{subfigure}
\caption{\textit{Left}: the ORF reconstructed checkerboard (light blue) is noisier than the stereo one (dark blue) in the $Z$ direction. \textit{Right}: this effect is less pronounced in the $XY$ plane. The holes are due to rejection of the points considered as too noisy.}
\label{fig:manip_2_4}
\end{center}
\end{figure}
...
\newpage
\noindent
In the next part of the process, the sum of the distances between the range finder and the stereo device is minimized and extrinsic matrices are computed. Here, we display the $M_{TL}$ matrices of a calibration using 7 checkerboard capture with the sensors plugged on the Halo. With RANSAC:
\begin{equation}
\begin{pmatrix}[0.7]
...
\begin{itemize}
\item \textbf{Stereo calibration parameters dependency}: First, as we just reminded, the stereo precision is a function of the \textit{focal length} $f$ and the \textit{baseline} $b$ between left and right cameras. Yet, those values are extracted from the stereo calibration and small imprecision can strongly affect the error on the final Halo calibration. Indeed, during the \textit{triangulation}, errors on $b$ and $f$ will cause the reconstructed points cloud to inflate or deflate (figure \ref{fig:manip_2_5}). As the minimization process of this calibration calculates the rotation and translation between the geometric center of each cloud, the calibration will be then dependent on the localization of observed checkerboards (directly influencing the position of the cloud's geometric centers). A checkerboard situated essentially along the $X$ axis would lead to a higher error on $X$. In our example, we observe mainly checkerboard along $Z$ but one of them is shifted on the $Y$ axis, which explain the higher error on $Y$. It is important to understand that this problem is only due to the stereo imprecision, though. Because of this effect, the global observation drawn from all the calibration tests shows a bigger error in $Z$. As a matter of fact, if the checkerboards photographed during the calibration process are near the center of the $XY$ plane, they by necessity have a positive $Z$ coordinate.
...
\end{subfigure}
\caption{A typical sample of input pictures for fusion. \textit{From left to right}: ORF depth, ORF visual, ORF confidence, left image, right image.}
\label{fig:manip_3_1}
\end{center}
\end{figure}
...
\end{subfigure}
\caption{\textit{Left}: in blue, the 3D ORF points; in green, a interval has been constructed around those points in function of the noise. \textit{Right}: in blue and green, idem; in red, the 3D points computed in the end of the fusion.}
\label{fig:manip_3_3}
\end{center}
\end{figure}
...
\end{subfigure}
\caption{\textit{Left}: a flat checkerboard before fusion. \textit{Right}: the same checkerboard after the fusion. Unlike the first theoretical assumptions, the fusion algorithm produces noise.}
\label{fig:manip_3_5}
\end{center}
\end{figure}
...
As for the ground pictures, the fusion algorithm has been tested in a zero gravity environment provided by the NASA \gls{RGA}. To be able to use the data collected further in the project, the thermocam acquisition has been also added. To better understand the intent of this experiment, it may be important to remind the context of the project. Indeed, as mentioned in the introduction, this fusion algorithm is part of a \gls{SLAM} algorithm aiming at localizing, mapping and understanding the motion parameters of the objects around. Moreover, this algorithm is intended to be part of a control process meant for operation in an environment that cannot be reproduced outside a zero gravity environment. Tests in a  parabolic flights are then important to prove the reliability of the sensor fusion and acquisition as part as a full system as well as its performances in front of six \gls{DoF} moving objects. However, given the amount of data, lot of post-processing has still to be done but we can already see the complementarity of \gls{VERTIGO} and \gls{ORF} in the datasets meaning that an adapted algorithm could recover the best knowledge of the aircraft inside by using stereo and \gls{ToF} cameras in the same time.
...
If this algorithm is supposed to enhance the accuracy of 3D cloud built with the \gls{ORF} measurements, the image capture makes it impossible for the stereo system to recover from bad images obtained by by the \gls{ORF}. This has been mainly observed during \gls{RGA} tests sessions in which saturation, significant relative speeds, and a too short range have provided bad results but that could be improved using \gls{VERTIGO} reliability. Using the notations of section \ref{subsec:fusion:overview}, we can say that \textbf{certainty} is not assured to increase with the fusion.
...
\item \textbf{Spatial division}: The idea is to reconstruct two parallel 3D clouds with VERTIGO and the ORF in two different streams then use confidence and range to split the space into different parts. This time, the fusion is only temporal, which don not really match the goal neither.
\item \textbf{Other methods}: Quantity of promising methods have been found in \cite{stereo_tof_taxonomy} in the end of this internship. When moving forward in the \gls{INSPECT} project, refinements could be done to select the most interesting. To name but one, \cite{fin} propose a fusion method adapted for fast \gls{SLAM} algorithms.
\end{itemize*}
